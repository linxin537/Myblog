...
<template>
  <div style="max-width: 800px; margin: 0 auto; padding-top: 24px;">
    <div :style="{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }">
      <NText tag="h2" :style="{ fontSize: '24px', fontWeight: 700 }">分类管理</NText>
      <NButton type="primary" :style="{ borderRadius: '8px' }" @click="openCreate">新增分类</NButton>
    </div>
    ...
    <NModal v-model:show="showModal" :title="editingId ? '编辑分类' : '新增分类'">
      <div class="card" :style="{ padding: '24px', width: '420px', maxWidth: '90vw' }">
        <NForm ref="formRef" :model="form" label-placement="top">
          ...
          <NFormItem label="描述" path="description">
            <NInput v-model:value="form.description" type="textarea" :style="{ '--n-border-radius': '8px' }" />
          </NFormItem>
          <NFormItem label="排序" path="sort_order">
            <NInputNumber v-model:value="form.sort_order" :style="{ '--n-border-radius': '8px' }" />
          </NFormItem>
        </NForm>
